...
std::unique_ptr<Token> build_signed_number_parser(void) {
    ...
    auto sign = std::make_unique<Punctuation>();
...
std::unique_ptr<Token> build_comma_index_field_parser(void) {
    ...
    auto comma = std::make_unique<Punctuation>();
    comma->set_name("comma_index_field: comma");

    auto index = std::make_unique<Numeric>();
    index->set_name("comma_index_field: index");

    auto field = build_field_parser();
    field->set_name("comma_index_field: field");
...
std::unique_ptr<Token> build_address_parser(void) {
    ...
    comma_index_field->set_optional()->set_name("comma_index_field: parser");

    auto comma_index = build_comma_index_parser();
    comma_index->set_optional()->set_name("comma_index: parser");

    auto field_only = build_field_parser();
    field_only->set_optional()->set_name("field_only: parser");
